...
# 一键安装  
```
bash <(curl -Ls https://raw.githubusercontent.com/FranzKafkaYu/sing-box-yes/develop/install.sh)
```    
执行后会自行显示管理菜单，通过菜单选项`1`将会自动安装最新release版本。与此同时，你也可以通过`sing-box install`来安装最新版本。  

如果你想安装某个特定版本,请使用以下命令,将`1.0.3`替换为特定版本号即可。  
```
bash <(curl -Ls https://raw.githubusercontent.com/FranzKafkaYu/sing-box-yes/develop/install.sh) install 1.0.3
```
# 快捷方式
